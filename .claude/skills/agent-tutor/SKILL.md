...
1. Read the dashboard first (create from template if missing). If the learner profile sets `output_format.dashboard: html`, the state read is the `agent-tutor-state` island at the top of `Dashboard.html` (single read — see *Dashboard format* below; file missing → generate it). Otherwise read `Learning/Dashboard.md`.
