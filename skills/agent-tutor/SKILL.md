...
The switch is feature-detected and applies at the **next dashboard write** — no migration session, no version field. Detect it at session start: the profile's `output_format` disagrees with what the vault shows. `output_format.dashboard: html` with no `Dashboard.html` but a full markdown `Dashboard.md` → markdown → html switch (step 1 of *Session start* routes here). `dashboard: markdown` after html → the next dashboard write regenerates the full markdown `Dashboard.md` from the authorities; the last `Dashboard.html` and its subject pages stay as untouched history.

**Markdown → html, at the next dashboard write — in one pass:**

1. **Archive the old dashboard.** Rename the full markdown `Dashboard.md` → `Dashboard-archive-<today>.md` — a pure rename, never rewritten afterwards. Skip the archive when there is nothing real to keep: a dashboard freshly created from the template in this same session is simply replaced, and a `Dashboard.md` that is already the thin hub is just refreshed.
2. **Check links pointing into the dashboard.** Bare `[[Dashboard]]` wikilinks elsewhere in the vault still resolve — the thin hub inherits the `Dashboard.md` name. Anchored deep links (`[[Dashboard#📊 Progress]]`, `[[Dashboard#Reviews]]`, …) now dangle: the hub has no sections — repoint each one to the archive (`[[Dashboard-archive-<today>#📊 Progress]]`). Links *out of* the dashboard (into plans, notes, assets) are unaffected by the rename.
